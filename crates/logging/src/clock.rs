//! Provides Palace's process-wide local clock.
//!
//! This module does not use `OffsetDateTime::now_local()` because platform local-offset discovery
//! can return `IndeterminateOffset` in otherwise valid runtime environments. Palace instead resolves
...
impl Clock {
    ...
    fn timezone(&self) -> chrono_tz::Tz {
        match self.timezone.get() {
            ...
            None => panic!("palace-logging clock must be initialized before use"),
        }
    }
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "palace-logging clock must be initialized before use")]
