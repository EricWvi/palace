...
浏览器 secret 由独立部署密钥、Session UUIDv7 和轮换代次通过 HMAC-SHA256 生成，数据库仅保存摘要；refresh credential 使用 AES-256-GCM 和 Session UUIDv7 关联数据加密。部署必须持久保管同一密钥；密钥不能放入数据库或日志。旧 secret 只在轮换后 30 秒内接受，响应统一返回当前代次，避免并发标签页不断覆盖新 cookie。
